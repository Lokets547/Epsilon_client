...
import wtf.dettex.modules.impl.combat.TriggerBot;
...
public class TargetHud extends AbstractDraggable {
    ...
    private void drawMain(DrawContext context, MatrixStack matrix) {
        ...
        blurGlass.render(ShapeProperties.create(matrix, getX(), getY(), getWidth(), getHeight()).round(5).softness(1)
                .thickness(2).outlineColor(ColorUtil.getOutline()).color(ColorUtil.getRect(Hud.newHudAlpha.getValue())).build());
        ...
        if (font.getStringWidth(lastTarget.getName().getString()) > 60) {
            ...
            font.drawGradientString(matrix, lastTarget.getName().getString(), getX() + 34, getY() + 8, ColorUtil.getText(), ColorUtil.getText(Hud.newHudAlpha.getValue()));
            scissorManager.pop();
    ...
    private void drawArmor(DrawContext context, MatrixStack matrix) {
        ...
        if (!items.isEmpty()) {
            ...
            blurGlass.render(ShapeProperties.create(matrix, 0, 0, items.size() * 11, 11).round(2.5F).softness(1)
    ...
    private void drawUsingItem(DrawContext context, MatrixStack matrix) {
        ...
        if (!animation.isFinished(Direction.BACKWARDS) && !lastItem.equals(Items.AIR)) {
            ...
            MathUtil.setAlpha(anim, () -> {
                blurGlass.render(ShapeProperties.create(matrix, x, y, size, size).round(12).softness(1)
                        .thickness(2).outlineColor(ColorUtil.getOutline()).color(ColorUtil.getRect(0.7F)).build());
