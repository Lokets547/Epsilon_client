...
public class MultiSelectComponent extends AbstractSettingComponent {
    ...
    public MultiSelectComponent(MultiSelectSetting setting) {
        ...
        for (String value : setting.getList()) {
            multiSelectedButtons.add(new MultiSelectedButton(setting, value));
        }
    }
    ...
    private void renderSelected(MatrixStack matrices, float innerX, float innerW) {
        blurGlass.render(ShapeProperties.create(matrices, innerX, y + 14, innerW, SELECTOR_HEIGHT)
                .round(3).thickness(2).outlineColor(ColorUtil.getOutline()).color(ColorUtil.getRect(0.6F)).build());

        String selectedName = String.join(", ", setting.getSelected());
        var font = Fonts.getSize(12, BOLD);
        float maxWidth = Math.max(0, innerW - 6); // 3px отступ с каждой стороны
    ...
    private void renderSelectList(DrawContext context, int mouseX, int mouseY, float delta, float expandProgress) {
        ...
        blurGlass.render(ShapeProperties.create(context.getMatrices(), dropdownListX, dropDownListY, dropDownListWidth, currentHeight)
                .round(4).thickness(2).outlineColor(ColorUtil.getOutline(opacity, 1)).color(ColorUtil.getRect(0.6F)).build());
