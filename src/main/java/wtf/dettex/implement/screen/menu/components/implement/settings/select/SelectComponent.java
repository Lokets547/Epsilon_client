...
public class SelectComponent extends AbstractSettingComponent {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        // Layout: title on top, selector below, inner width (width-6)
        ...
        // selector box at y+18, dropdown below it with 2px gap
        ...
        // Animate height with dropdown progress
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (button == 0 || button == 1) {
            // Click on selector box toggles dropdown
    ...
    private void renderSelected(MatrixStack matrices, float innerX, float innerW) {
        ...
        blurGlass.render(ShapeProperties.create(matrices, innerX, y + 15, innerW, SELECTOR_HEIGHT)
                .round(2).thickness(2).outlineColor(ColorUtil.getOutline()).color(ColorUtil.getRect(0.6F)).build());
    ...
    private void renderSelectList(DrawContext context, int mouseX, int mouseY, float delta, float expandProgress) {
        ...
        scissorManager.push(matrix, dropdownListX, dropDownListY, dropDownListWidth, currentHeight);

        blurGlass.render(ShapeProperties.create(context.getMatrices(), dropdownListX, dropDownListY, dropDownListWidth, currentHeight)
                .round(3).thickness(2).outlineColor(ColorUtil.getOutline(opacity, 1)).color(ColorUtil.getRect(0.6F)).build());

        float offset = dropDownListY;
        float maxVisibleY = dropDownListY + currentHeight;
