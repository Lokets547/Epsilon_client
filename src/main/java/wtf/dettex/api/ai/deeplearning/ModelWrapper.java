...
public abstract class ModelWrapper<I, O> implements Closeable {
    ...
    public void load(Path path) throws IOException, ModelException {
        // Note: DJL Model.load expects a directory and a model name/prefix for artifact stores.
        // Our shipped ".params" are optional; skip if not present/compatible.
    ...
    public void loadFromResources(String resourcePath) {
        try (InputStream stream = getClass().getResourceAsStream(resourcePath)) {
            if (stream == null) {
                System.out.println("[AI] Model resource not found, skipping: " + resourcePath);
                return;
            }
            // Loading from InputStream is not supported for DJL models; skip to avoid crash.
            System.out.println("[AI] Stream-based model load not supported, skipping: " + resourcePath);
