...
import wtf.dettex.implement.features.draggables.CoolDowns;
import wtf.dettex.modules.impl.combat.killaura.rotation.*;
import wtf.dettex.modules.impl.combat.killaura.rotation.angle.ReallyWorldSmoothMode;
import wtf.dettex.modules.impl.render.ProjectilePrediction;
            ...
            if (autoLootSetting.isValue() && autoLootSetting.isVisible()) {
                PlayerIntersectionUtil.streamEntities().filter(MerchantEntity.class::isInstance).map(MerchantEntity.class::cast)
                        .filter(m -> m.hasStackEquipped(EquipmentSlot.MAINHAND) || m.hasStackEquipped(EquipmentSlot.OFFHAND)).findFirst().ifPresent(merchant -> {
                            Vec3d attackVector = pointFinder.computeVector(merchant, 6, RotationController.INSTANCE.getRotation(), new ReallyWorldSmoothMode().randomValue(), true).getLeft();
