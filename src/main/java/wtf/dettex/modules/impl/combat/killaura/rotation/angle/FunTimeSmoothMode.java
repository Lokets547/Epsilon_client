...
public class FunTimeSmoothMode extends AngleSmoothMode {
    ...
    public Angle limitAngleChange(Angle currentAngle, Angle targetAngle, Vec3d vec3d, Entity entity) {
        ...
        if (entity != null) {
            ...
            moveAngle.setYaw(MathHelper.lerp(randomLerp(speed, speed + 0.35F), currentAngle.getYaw(), currentAngle.getYaw() + moveYaw));
            moveAngle.setPitch(MathHelper.lerp(randomLerp(speed, speed + 0.35F), currentAngle.getPitch(), currentAngle.getPitch() + movePitch));

            return moveAngle;
        } else {
            int suck = count % 3;
            float speed = attackTimer.finished(400) ? new SecureRandom().nextBoolean() ? 0.4F : 0.2F : -0.2F;
            float random = attackTimer.elapsedTime() / 60F + (count % 6);
            ...
            Angle moveAngle = new Angle(currentAngle.getYaw(), currentAngle.getPitch());
            moveAngle.setYaw(MathHelper.lerp(Math.clamp(randomLerp(speed, speed + 0.35F), 0, 1), currentAngle.getYaw(), currentAngle.getYaw() + moveYaw) + yaw);
            moveAngle.setPitch(MathHelper.lerp(Math.clamp(randomLerp(speed, speed + 0.35F), 0, 1), currentAngle.getPitch(), currentAngle.getPitch() + movePitch) + pitch);
