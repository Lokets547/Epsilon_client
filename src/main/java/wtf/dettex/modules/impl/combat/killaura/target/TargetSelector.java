package wtf.dettex.modules.impl.combat.killaura.target;
...
import wtf.dettex.modules.impl.combat.killaura.rotation.PointFinder;
...
public class TargetSelector implements QuickImports {
    ...
    private double getFov(LivingEntity entity, float maxDistance, boolean ignoreWalls) {
        Vec3d attackVector = pointFinder.computeVector(entity, maxDistance, RotationController.INSTANCE.getRotation(), new ReallyWorldSmoothMode().randomValue(), ignoreWalls).getLeft();
    ...
    private Optional<LivingEntity> findFirstMatch(java.util.function.Predicate<LivingEntity> predicate) {
    ...
    public static class EntityFilter {
        ...
        private boolean isInvisiblePlayer(PlayerEntity player) {
        ...
        private boolean isValidEntityType(LivingEntity entity) {
            return switch (entity) {
                case PlayerEntity player -> {
                    if (FriendUtils.isFriend(player) && !targetSettings.contains("Friends")) {
                        yield false;
                    }

                    boolean playersSelected = targetSettings.contains("Players");
                    boolean invisibleSelected = targetSettings.contains("Invisible");
                    boolean nakedInvisibleSelected = targetSettings.contains("Naked Invisible");
                    boolean invisible = isInvisiblePlayer(player);

                    boolean visiblePlayerAllowed = playersSelected && !invisible;
                    boolean invisiblePlayerAllowed = invisibleSelected && invisible;
                    boolean nakedInvisibleAllowed = nakedInvisibleSelected && invisible && hasNoArmor(player);

                    yield visiblePlayerAllowed || invisiblePlayerAllowed || nakedInvisibleAllowed;
                }
