package wtf.dettex.common.util.color;
...
public class ColorUtil {
    ...
    public int replAlpha(int color, float alpha) {
        return getColor(red(color), green(color), blue(color), Math.round(alpha * 255));
    }

    public static int interpolateColor(float progress, int from, int to) {
        float clamped = MathHelper.clamp(progress, 0.0F, 1.0F);
        int a = (int) MathHelper.lerp(clamped, alpha(from), alpha(to));
        int r = (int) MathHelper.lerp(clamped, red(from), red(to));
        int g = (int) MathHelper.lerp(clamped, green(from), green(to));
        int b = (int) MathHelper.lerp(clamped, blue(from), blue(to));
        return ColorHelper.getArgb(a, r, g, b);
    }

    public int multColor(int colorStart, int colorEnd, float progress) {
        return interpolateColor(progress, colorStart, colorEnd);
    }
    ...
    public Vector4i roundClientColor(float alpha) {
        return new Vector4i(multAlpha(fade(270), alpha), multAlpha(fade(0), alpha),
                multAlpha(fade(180), alpha), multAlpha(fade(90), alpha));
    }
    ...
    public int getRect(float alpha) {return multAlpha(new Color(0x000000).getRGB(),alpha);}

    public int getAltManager(float alpha) {
        return multAlpha(new Color(0x233043).getRGB(),alpha);
    }
